...
def prediction(path_dir, device, MLP_Batch_full, MLP_meta_full, MLP_TE_full, EPOCHS):
    AENet = torch.load(join(path_dir, 'AE_pretrain/AE_'+EPOCHS+'_pretrained.pt'))

    MLP = torch.load(join(path_dir,'MLP/MLP_'+EPOCHS+'_'+EPOCHS+'.pt'))
...
def make_prediction(data_mode, bin_size, proportion, path_to_TE_ref, trained_epochs, sample = None, USE_GPU= torch.cuda.is_available()):
    ...
    if data_mode =='Smart_seq':
        ...
        p = cur_path + '/Multi_TE'
        MLP_TE_full = scipy.sparse.load_npz(join(p,"Multi_TE_full_"+bin_size + '_' + proportion +".npz")).toarray()
        MLP_Batch_full = scipy.sparse.load_npz(join(p,"Multi_Batch_full_encode_" +bin_size + '_' + proportion +".npz")).toarray()
        with open(join(p,'Multi_meta_full_'+bin_size + '_' + proportion +'.pkl'), 'rb') as f:
            MLP_meta_full=pickle.load(f)
        path_dir = join(os.getcwd(), 'training_'+bin_size + '_' + proportion)
        Meta_Data_full, problist_full = prediction(path_dir, DEVICE, MLP_Batch_full, MLP_meta_full, MLP_TE_full, trained_epochs)
